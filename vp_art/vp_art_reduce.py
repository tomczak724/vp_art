...
master_targets = []
master_targets_txt = open(working_dir + '/TARGETS.list')
...
for night in nights:
    ...
    ######################################################
    os.chdir(night)
    night_targets = np.loadtxt(working_dir + '/redux/' + night + '/' +
                               'targets.txt', dtype=str)
    ...
    if params.FLAT_COMBINE=='y':
        os.chdir(working_dir + '/redux/' + night + '/calib/flat/')
        flat_ims = gl.glob('*fits')
        flat_stk = im_stack.flat_stack(flat_ims)
        flat_exists = len(gl.glob(working_dir + '/redux/' +
                                  night + '/' + flat_stk)) == 1
        if flat_exists:
            print '\n\t   PREVIOUS FLAT-STACK ALREADY EXISTS... CLOBBERING\n'
            os.remove(working_dir + '/redux/' + night + '/' + flat_stk)
        flat_exists = True
        shu.move(flat_stk, working_dir + '/redux/' + night + '/')
        os.chdir(working_dir + '/redux/' + night)
    else:
        t = time.gmtime()
        h,m,s = str(t.tm_hour), str(t.tm_min), str(t.tm_sec)
        if len(h)==1: h='0'+h
    ...
    if params.COMP_COMBINE=='y':
        os.chdir(working_dir + '/redux/' + night + '/calib/comp/')
        comp_ims = gl.glob('*fits')
        comp_stk = im_stack.comp_stack(comp_ims, params.COMP_HEADER, params.COMP_NAME)
        comp_exists = len(gl.glob(working_dir + '/redux/' +
                                  night + '/' + comp_stk)) == 1
        if comp_exists:
            print '\n\t   PREVIOUS COMP-STACK ALREADY EXISTS... CLOBBERING\n'
            os.remove(working_dir + '/redux/' + night + '/' + comp_stk)
        comp_exists = True
        shu.move(comp_stk, working_dir + '/redux/' + night + '/')
        os.chdir(working_dir + '/redux/' + night + '/')
    else:
        t = time.gmtime()
        h,m,s = str(t.tm_hour), str(t.tm_min), str(t.tm_sec)
    ...
    if params.FIBER_TRACE=='y':
        ...
        comp_names = [params.COMP_NAME[i*2:(i + 1)*2]
                      for i in range(len(params.COMP_NAME) / 2)]
        lam.interactive_initial_guess(comp_stk, 'trace_pass1.fits', comp_names)
        print '\n\tDONE WITH INTERACTIVE WAVELENGTH SOLUTION'



        trc_files = gl.glob(fiber_folder+'/fibertrace_id*')
        if not len(trc_files): fibers = trc.calc_trace_align(flat_stk, fibers,
                                                             fiber_folder, params)
        else: fibers = trc.read_fibers_from_txt(trace_files, fiber_folder
                                                             +'/fibertrace_ys.txt')
        print ''
    ...
    if params.BIAS_COMBINE=='y':
        os.chdir(working_dir + '/' + night + '/calib/bias/')
        bias_ims = gl.glob('*fits')
        bias_stk = im_stack.bias_stack(bias_ims)
        bias_exists = len(gl.glob(working_dir + '/redux/' + night + '/' + bias_stk))==1
        if bias_exists:
            print '\n\t   PREVIOUS BIAS-STACK ALREADY EXISTS... CLOBBERING\n'
            os.remove(working_dir + '/redux/' + night + '/' + bias_stk)
        shu.move(bias_stk, working_dir + '/redux/' + night + '/')
        bias_exists = True
        os.chdir(working_dir + '/redux/' + night)
    else:
        t = time.gmtime()
    ...
    if params.LAMBDA_SOLVE=='y':
        ...
        if len(lambdafiles)==0:
            ...
            linesdat = np.loadtxt(working_dir + '/redux/' + night + '/initial_lambda_soln.dat')
